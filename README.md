...
4. **`internal_monologue.jsonl` (内在独白)**：那些没有直接发给你的“内心 OS”，比如：_（其实刚刚在逛东西，看到一个很有意思的说法）_。
5. **`autonomous_state.jsonl` (自主状态)**：当你不理它的时候，定时任务（Life Tick）会让它自己去“看书”、“看B站科普视频”甚至“无聊发呆”，并且判断是否要主动去戳你。
